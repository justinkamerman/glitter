...
#----------------------------------------
    ...
    if ( parent.followers_count > (cache_count*1.1) ):
        print ("Incomplete follower data for user %s. Have %d followers but should have %d (exceeds margin for error)." 
    ...
    while True:
        try:
            ...
            try:
                ...
                print ("Caught TweepError checking rate limit: %s" % (err))
                print ("Sleeping a few seconds and then retrying")
                time.sleep (5)
...
retweets = api.retweets (status.id, count=100)
